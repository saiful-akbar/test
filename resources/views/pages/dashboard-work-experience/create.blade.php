...
@section('content')
    <div class="row mb-3">
        <div class="col-sm-12">
            <form action="{{ route('dashboard.work-experience.store') }}" method="POST">
                @csrf @method('POST')
                                ...
                                <div class="@error('from_period') is-invalid @enderror">
                                    ...
                                    >
                                        ...
                                        @for ($i = date('Y'); $i > date('Y') - 100; $i--)
                                            <option value="{{ $i }}" {{ old('from_period') == $i ? 'selected' : null }} >{{ $i }}</option>
                                        @endfor
                                    </select>
